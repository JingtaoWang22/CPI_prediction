...
class CompoundProteinInteractionPrediction(nn.Module):
    def __init__(self):
        ...
        self.embed_fingerprint = nn.Embedding(n_fingerprint, dim)             # flaw1: should decode and get true structure? -JW
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model, dropout, max_len=13101):
...
if __name__ == "__main__":
    ...
    (DATASET, dataname,radius, ngram, dim, heads, dim_ff, dim_ff2,N,M, layer_gnn, window, layer_cnn, lr, lr_decay,
     decay_interval, iteration, setting) = sys.argv[1:]

    (dim, heads, dim_ff,dim_ff2,N,M, layer_gnn, window, layer_cnn, decay_interval,
     iteration) = map(int, [dim, heads, dim_ff,dim_ff2,N,M, layer_gnn, window, layer_cnn, decay_interval,
                            iteration])
   ...
   # print("dataset_test")
    ...
    file_result = '../output/result/' +dataname + " " + setting +'originTcnorm.txt'
    with open(file_result, 'w') as f:
        f.write('Epoch\tTime(sec)\tLoss_train\tAUC_dev\t'
                'AUC_test\tPrecision_test\tRecall_test\n')

    file_model = '../output/model/' + setting+dataname+"c_norm"
